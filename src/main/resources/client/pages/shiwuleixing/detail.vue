	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('shiwuleixing', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','shiwuleixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','shiwuleixing');
			...
			async init(){
				...
				let res = await this.$api.info('shiwuleixing', this.id);
				this.detail = res.data;
			},
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussshiwuleixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
