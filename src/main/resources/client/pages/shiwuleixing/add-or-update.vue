	...
	<view :style='{"width":"100%","padding":"0 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"24rpx 24rpx 24rpx 24rpx","background":"#ffffff","display":"block","height":"auto"}' class="app-update-pv">
			<view :style='{"padding":"4rpx 0 4rpx 0","margin":"0 0 24rpx 0","borderColor":"#cccccc","alignItems":"center","borderRadius":"8rpx 0 0 8rpx","borderWidth":"0px","background":"#e7fefc","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#333333","textAlign":"right"}' class="title">食物类型</view>
				<input :style='{"border":"0px solid #eeeeee","padding":"0px 24rpx 0px 24rpx","margin":"0 0 0 0","color":"#666666","borderRadius":"8rpx","flex":"1","background":"#ffffff00","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx"}' :disabled="ro.shiwuleixing" v-model="ruleForm.shiwuleixing" placeholder="食物类型"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`shiwuleixing`, this.ruleForm.id);
...
//跨表计算判断
				var obj;
				if((!this.ruleForm.shiwuleixing)){
					this.$utils.msg(`食物类型不能为空`);
					return
				}
				//更新跨表属性
			       var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`shiwuleixing`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`shiwuleixing`, this.ruleForm);
						}else{
							await this.$api.add(`shiwuleixing`, this.ruleForm);
						}
