		...
		<view class="container" :style='{"width":"100%","padding":"0px","position":"relative","background":"#ffffff","height":"auto"}'>
            <view :style='{"padding":"24rpx 24rpx 24rpx 24rpx","flexWrap":"wrap","background":"#ffffff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"width":"100%","padding":"24rpx 24rpx 24rpx 24rpx","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx 0 20rpx","margin":"0 20rpx 12rpx 0","color":"#ffffff","background":"#ffaa33","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('yundongjilu','热量')" @tap="onAcrossTap('yundongreliang','','','')" class="cu-btn bg-brown round shadow-blur" >热量</button>
					<button :style='{"border":"0","padding":"0 20rpx 0 20rpx","margin":"0 20rpx 12rpx 0","color":"#ffffff","background":"#ffaa33","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('yundongjilu','热量')" @tap="onAcrossTap('yundongreliang','','','')" class="cu-btn bg-brown round shadow-blur" >热量</button>
				</view>
			</view>
	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('yundongjilu', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','yundongjilu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','yundongjilu');
			...
			async init(){
				...
				let res = await this.$api.info('yundongjilu', this.id);
				this.detail = res.data;
				//修改富文本的图片样式
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussyundongjilu/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
